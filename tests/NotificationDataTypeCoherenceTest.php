...
it('every NotificationType data class returns its own notificationType without exception', function (NotificationType $type): void {
    ...
    $minimalArgs = match ($type) {
        ...
        NotificationType::MarketplaceUserReassignNotification => ['https://example.test', 4321, 'https://marketplace.example/demands/4321'],
        NotificationType::MarketplaceAssignationActivationNotification => ['https://example.test', 4321, 'https://example.test/demands/4321'],
        NotificationType::MarketplaceNewChatMessageForCustomerNotification => ['https://example.test', 4321, '98765', 'claim-token'],
