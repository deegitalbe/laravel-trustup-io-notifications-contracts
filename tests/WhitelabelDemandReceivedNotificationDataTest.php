...
it('builds WhitelabelDemandReceivedNotificationData from its fields', function (): void {
    $data = new WhitelabelDemandReceivedNotificationData(
...
it('round-trips WhitelabelDemandReceivedNotificationData via toArray and fromArray', function (): void {
    $original = new WhitelabelDemandReceivedNotificationData(
        base_url: 'https://example.test',
        demand_id: 4321,
        ai_session_id: 'sess_9f3ba71c',
        claim_token: 'tok_9f3ba71c',
    ...
    $restored = WhitelabelDemandReceivedNotificationData::fromArray($original->toArray());
...
it('keeps demand_id an integer across a real JSON round-trip', function (): void {
    ...
    $restored = WhitelabelDemandReceivedNotificationData::fromArray($decoded);
...
it('rejects a demand_id that arrives as a numeric string rather than coercing it', function (): void {
    expect(fn () => WhitelabelDemandReceivedNotificationData::fromArray([
        'base_url' => 'https://example.test',
        'demand_id' => '4321',
        'ai_session_id' => 'sess_9f3ba71c',
        'claim_token' => 'tok_9f3ba71c',
...
it('defaults ai_session_id, claim_token and first_name to null when missing from the payload', function (): void {
    $restored = WhitelabelDemandReceivedNotificationData::fromArray([
        'base_url' => 'https://example.test',
        'demand_id' => 4321,
    ]);
...
it('reports the whitelabel demand-received notification type', function (): void {
    $data = new WhitelabelDemandReceivedNotificationData(
        base_url: 'https://example.test',
        demand_id: 4321,
        ai_session_id: 'sess_9f3ba71c',
    ...
    expect($data->notificationType())->toBe(NotificationType::WhitelabelDemandReceivedNotification);
});
